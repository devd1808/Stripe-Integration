...
try {
    ...
    // If customer does not exist, create new Stripe customer and store in DB
    // if (!$customerid) {
        ...
        $stmt = $conn->prepare("INSERT INTO customers (user_id, stripe_customer_id, email) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $userid, $customerid, $email);
        $stmt->execute();
        $stmt->close();
    // }
